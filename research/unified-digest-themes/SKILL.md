...
| Skill | How to Apply |
|-------|-------------|
| hn-brief-digest | Load unified-digest-themes alongside hn-brief-digest. Group HN stories under themes; use sub-themes within AI & ML Research when 3+ stories qualify. |
| x-digest | Load unified-digest-themes alongside x-digest. Tag tweets with themes during summarization. |
| smol.ai news (weekly/monthly) | Load unified-digest-themes alongside the primary prompt. Classify AI news items into themes for structured reporting. |
| arxiv-daily-papers | Load unified-digest-themes alongside arxiv skill. Assign papers to themes for themed grouping. |
...
- [AI & ML Research Sub-Theme Reference](references/ai-ml-research-sub-themes.md) — Detailed overlap guidance and decision rules for sub-theme classification.
- [Active Cron Jobs Using This Theme System](references/active-cron-jobs.md) — Every cron job that loads unified-digest-themes. Update this when adding/removing cron jobs or changing the taxonomy.
