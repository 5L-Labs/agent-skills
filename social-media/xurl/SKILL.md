...
**Important for headless/agent environments:** The OAuth 2.0 flow opens a browser automatically. If you're running in a headless server, you'll need to:
1. Copy the URL displayed when you run `xurl auth oauth2`
2. Paste it into a browser on another machine or use a text-based browser (like `w3m` or `lynx`) if available
3. Complete the authorization
4. Return to your terminal and paste the verification code

Alternatively, if you have pre-generated OAuth 1.0a credentials (consumer key/secret and access token/secret), you can configure them directly without a browser flow:
...
These credentials are saved to your local xurl configuration and can be used immediately. Verify with:

```bash
xurl whoami
```

Note: OAuth1 credentials are tied to a specific user account and provide full user context. Ensure you keep these credentials secure.
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `Unsupported Authentication` or 403 on `/2/users/me` with bearer token | Using Application-Only (app-only) bearer token instead of User Context token | Use OAuth 2.0 User Context flow or OAuth 1.0a credentials instead. App-only tokens work only for app-only endpoints like `/2/users/me` with app-only scope, but not for user-requiring endpoints. |
| OAuth 2.0 flow hangs or times out in headless/agent environment | Browser-based PKCE flow requires user interaction | Complete the OAuth flow manually outside the agent session. Visit the provided URL, authenticate, and enter the code. Alternatively, use pre-generated OAuth 1.0a credentials if available. |
