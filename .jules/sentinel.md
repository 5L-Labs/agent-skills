...
## 2026-08-08 - Fixed Bandit B108 (hardcoded tmp directory) in pytest
**Vulnerability:** A test script (`media/wsj-reader/tests/test_cli.py`) used a hardcoded temporary directory (`/tmp/wsj-profile`). In a shared environment, an attacker could pre-create this directory or file, potentially causing test failures or exploiting symlink attacks if the test doesn't handle existing paths securely.
**Learning:** Hardcoded paths in tests, especially in `/tmp/`, can lead to insecure temporary file usage (Bandit B108).
**Prevention:** Always use pytest's built-in `tmp_path` fixture to dynamically generate secure, isolated temporary paths for testing (e.g., `str(tmp_path / "wsj-profile")`).
